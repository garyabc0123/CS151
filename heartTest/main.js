...
function update(state){
    if(typeof state === 'number'){
        if(state == 0){
            ...
            $("#question-content").empty();
            $("#title-block").empty();
            $("#but").attr("class", "form-check");

            $("#start-button").text("下一步");
            $("#title-block").append(questions[state - 1]["question"] );
            $("#flexRadioDefault1Text").text(questions[state -1]["answers"][0][0]);
            $("#flexRadioDefault2Text").text(questions[state -1]["answers"][1][0]);
            $("#flexRadioDefault3Text").text(questions[state -1]["answers"][2][0]);

        }
    }else{
        $("#question-content").empty();
        $("#title-block").empty();
        $("#but").attr("class", "form-check d-none");
        $("#start-button").text("重新開始");
        $("#title-block").append(finalAnswers[state][0] );
        $("#question-content").append(finalAnswers[state][1]);
        ...
        if(typeof nowstate === 'number'){
            if(nowstate == 0){
                ...
                $.each($(":radio"), function (i, val){
                    if(val.checked){
                        nowstate = questions[nowstate - 1]["answers"][i][1];
                    }
                });
                update(nowstate);
            }
